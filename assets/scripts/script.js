...
// Установить картинки для пунктов меню при загрузке страницы
cat > assets/scripts/script.js <<'EOF'
const glassOptions = [
  'Прозрачное',
  'Пепельное',
  'Йодовое',
...
if(step2El){
  step2El.addEventListener('click', ()=>{
    ...
    const target=document.getElementById('subsystems');
    scrollToEl(target);
  });
}
...
if(step3El){
  step3El.addEventListener('click', ()=>{
    ...
    const target=document.getElementById('glass-options');
    scrollToEl(target);
  });
}

if(step4El){
  step4El.addEventListener('click', ()=>{
    if(menuDiv){
      menuDiv.classList.add('hidden');
    }
    if(formContainer){
    ...
    const target=document.getElementById('shotlan-options');
    scrollToEl(target);
  });
}

// optimize media loading
document.querySelectorAll('video.icon').forEach(v=>{
  v.preload = 'none';
